...
def generate_status_body(approval_manager: ApprovalManager, status: str, quorum: int) -> str:
    """Generate the status body for RFC approval"""
    ...
    body += f"RFC has approvals from {len(approval_manager.core_approvals)}/{quorum} required @nf-core/core quorum.\n\n"
...
def main(github_token, org, repo, issue_number, event_name, event_action, issue_state, label_name, issue_state_reason):
    """Process RFC proposal approval automation."""
    print(f"Processing RFC approval for issue #{issue_number}")
    ...
    quorum = math.ceil(len(approval_manager.core_team_members) / 2)
    print(f"Quorum set to {quorum}.")
    ...
    if event_name == 'issues' and event_action in ['labeled', 'unlabeled']:
        if label_name == 'timed-out':
            ...
            status_body = generate_status_body(approval_manager, '⏰ Timed Out', quorum)
            approval_manager.update_status_comment(status_body)
            return

    # Handle new issue creation
    if event_name == 'issues' and event_action == 'opened':
        body = generate_status_body(approval_manager, '🕐 Pending', quorum)
    ...
    if (event_name == 'issues' and
        ...
        status = '❌ Rejected'
    elif len(approval_manager.core_approvals) >= quorum:
        status = '✅ Approved'

    status_body = generate_status_body(approval_manager, status, quorum)
